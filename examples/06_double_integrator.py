...
# Importa le classi e le funzioni dai file forniti
# Si assume che i file siano in una cartella chiamata 'ACMPC' e 'DifferentialMPC'
# come suggerito dagli import nello script originale.
...
def f_dyn_batched(x, u, dt=DT):
    """Dinamica del doppio integratore, gestisce input batchati."""
    p, v = torch.split(x, 1, dim=-1)
    # Assicura che l'input di controllo abbia lo stesso tipo di dato dello stato
    u = u.to(p)
    p_new = p + v * dt
    v_new = v + u * dt
...
def f_dyn_jac_batched(x, u, dt=DT):
    """Jacobiana analitica della dinamica, gestisce input batchati."""
    B = x.shape[0]
    # Costruisce le matrici A e B e le espande per il batch
...
class DoubleIntegratorEnv(gym.Env):
    """Ambiente Gym per il doppio integratore."""
    def __init__(self):
        super().__init__()
        self.state = None
    ...
    def reset(self, initial_state=None):
        if initial_state is not None:
            ...
            self.state = (np.random.rand(NX) * 4 - 2).astype(np.float32)
        return self.state
    ...
    def step(self, action):
        ...
        reward = reward_fn_batched(state_tensor.unsqueeze(0), action_tensor.unsqueeze(0), next_state_tensor.unsqueeze(0)).item()
        self.state = next_state_tensor.numpy()
        return self.state, reward, False, {}


def env_factory():
    """Funzione factory per creare istanze dell'ambiente."""
    return DoubleIntegratorEnv()


def evaluate_and_plot(actor, env, device, file_prefix, title):
    """Esegue una valutazione della policy e genera i grafici."""
    ...
    for _ in range(100):
        ...
        with torch.no_grad():
            action, _, _, _ = actor(state_tensor, deterministic=True)
    ...
    axs[0].plot(time, states[:, 0], 'b', label='Posizione (p)'); axs[0].set_ylabel('Posizione'); axs[0].legend(); axs[0].grid(True, ls=':')
    axs[1].plot(time, states[:, 1], 'g', label='Velocità (v)'); axs[1].set_ylabel('Velocità'); axs[1].legend(); axs[1].grid(True, ls=':')
    axs[2].plot(time[:-1], actions[:, 0], 'r', drawstyle='steps-post', label='Controllo (a)'); axs[2].set_ylabel('Controllo'); axs[2].set_xlabel('Tempo (s)'); axs[2].legend(); axs[2].grid(True, ls=':')
    plt.tight_layout(rect=[0, 0, 1, 0.96]); plt.savefig(f"plots/{file_prefix}_trajectory.png"); plt.close(fig)

    # Plot del ritratto di fase
    ...
    ax.set_title(f'Ritratto di Fase: {title}'); ax.set_xlabel('Posizione (p)'); ax.set_ylabel('Velocità (v)'); ax.legend(); ax.grid(True)
    plt.tight_layout(); plt.savefig(f"plots/{file_prefix}_phase_portrait.png"); plt.close(fig)
    print(f"Grafici salvati in 'plots/{file_prefix}_*.png'")


if __name__ == "__main__":
    DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
    MPC_HORIZON, NUM_ENVS, EPISODE_LEN, TRAINING_STEPS, HISTORY_LEN = 15, 32, 50, 100, 25
    ...
    # --- MODIFICA CHIAVE ---
    # Rimuoviamo la classe wrapper `ActorMPCWithJacobian` e istanziamo `ActorMPC` direttamente,
    # passando la funzione jacobiana analitica. La classe base gestirà la configurazione corretta.
    actor = ActorMPC(
        nx=NX,
        nu=NU,
        horizon=MPC_HORIZON,
        dt=DT,
        f_dyn=f_dyn_batched,
        f_dyn_jac=f_dyn_jac_batched,  # Passiamo la jacobiana qui
        device=DEVICE
    ...
    checkpoint_manager.load(device=DEVICE, resume_from="best", actor=actor)
